<?php // time ago units class


if ( !defined('ABSPATH') ) exit;


class Time_Ago_Units {
  public $settings;

  public function __construct() {
    $this->settings = new Time_Ago_Units_Settings(); // add settings
    // add 'format date' action
    add_action( 'get_the_date', array ($this, 'time_ago_units_format_date'), 10, 3 );
  }


  // the meat of the plugin. Reformat the post date
  public function time_ago_units_format_date( $the_date, $d, $post ) {
    // Get the post id
    if ( is_int( $post) ) {
      $post_id = $post;
    } else {
  ...
  public function time_ago_units_default_format( $post_id ) {
  ...
  public function time_ago_units_no_mins_format( $datestring ) {
  ...
  public function time_ago_units_no_hours_format( $datestring ) {
  ...
  public function time_ago_units_no_days_format( $datestring ) {
    if ( strpos( $datestring, 'hour' ) !== false ||
         strpos( $datestring, 'min' ) !== false ||
  ...
  public function time_ago_units_no_weeks_format( $datestring ) {
  ...
  public function time_ago_units_no_months_format( $datestring ) {
    if ( strpos( $datestring, 'hour' ) !== false ||
         strpos( $datestring, 'min' ) !== false ||
         strpos( $datestring, 'day' ) !== false ||
  ...
  public function time_ago_units_prettify( $datestring ) {
